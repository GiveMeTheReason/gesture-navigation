...
RUN python3 -m pip install --no-cache-dir numpy matplotlib
...
COPY . gestures_navigation

WORKDIR /root/project/gestures_navigation

ENV PYTHONPATH=.
RUN ["chmod", "+x", "data/create_sub_dataset.sh"]
# ENTRYPOINT ["data/create_sub_dataset.sh"]
...
#     python3 -m pip install --no-cache-dir --upgrade open3d==0.16.0

# COPY . gestures_navigation

# WORKDIR /home/project/gestures_navigation
# ENV PYTHONPATH=.
# RUN ["chmod", "+x", "data/create_sub_dataset.sh"]
# ENTRYPOINT ["data/create_sub_dataset.sh"]
...
# # RUN adduser -u 5678 --disabled-password --gecos "" appuser && chown -R appuser /gestures_navigation
# # USER appuser

# # WORKDIR /gestures_navigation/
# # ENTRYPOINT ["python3", "/gestures_navigation/render_to_image.py"]
